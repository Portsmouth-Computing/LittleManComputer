...
class LMCParser:
    ...
    def parse_line(self, line, memory_location, memory):
        ...
        if instruction == "INP":
            ...
            # INP, OUT, and HLT are special cases, as they do not have an address to manip
            ...
            if instruction in self.commands:
                self.load_instruction(
                    memory, self.commands[instruction], line[1], memory_location)
            else:
                if (line[1] == "DAT"):  # Dat, takes the form of NAME DAT INITAL VALUE
                    if(len(line) == 2):
                        # a value is optional, so default to 0
