...
class Window(Frame):
    ...
    def load_instructions(self):
        ...
        # get rid of the final char, which is random space for some reason
    ...
    def init_window(self):
        ...
        self.assemble = Button(
            self, text="Load", command=self.load_instructions)
            ...
            for y in range(0, 20):
                ...
                if y % 2 == 0:
                    ...
                    self.memory_label.grid(
                        row=1 + (y), column=6 + (x), ipadx=5, ipady=2)
                else:
                    value_var.set(self.memory[loc_var])
                    self.memory_value = Label(
                        self, textvariable=value_var, bg='grey')
